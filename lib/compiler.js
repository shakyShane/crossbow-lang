...
(function(root, factory) {
  ...
  compiler.compile = function(source, name) {
    ...
    try {
      ...
      require("fs").writeFileSync("./funcs.js", compile(ast, name));
  ...
  compiler.optimizers = {
    ...
    after:     noop,
    format:    dust.config.stripWhitespace ? nullify : noop,
    reference: visit,
    '#':       visit,
    '?':       visit,
    '^':       visit,
  ...
  compiler.nodes = {
    ...
    '@': function(context, node) {
      ...
      /** If it's a highlight block, shift the pointer
       * to get the correct "after" text
       * @type {number}
       */
      var start = 1;
      if (node[1].text === "hl") {
      ...
      var after = escape(getAfterBuffer(node, start));

      return '.helper(' +
        escape(node[1].text) +
        ',' + compiler.compileNode(context, node[2]) + ',' +
        compiler.compileNode(context, node[4]) + ',' +
        compiler.compileNode(context, node[3]) + ',' + after + ')';
    },
   ...
   *
   */
  function getAfterBuffer(node, start) {
    var buffer = node[5][start || 1];
    if (buffer) {
